...
public class BestMatchingDeals {
	...
	public void initGrapheTauxDeChange(
		...
		for (Market m : markets) {
			...
			for (int t = 2; t < listCurrencies.size() - 1; t++) {
				for(Market m:markets){
				...
				CustomVertex sommetArrivee_t = new CustomVertex(monnaieDepart,t);
				
				for (Market m : markets) {

					if (m.cur1.equals(monnaieDepart)) {
						CustomVertex precedentArrivee_t = new CustomVertex(m.cur2, t-1);
